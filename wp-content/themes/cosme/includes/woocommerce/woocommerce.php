...
if ( ! function_exists( 'cosme_woocommerce_header_cart' ) ) {
	...
	function cosme_woocommerce_header_cart() {
		...
		$cosme_enable_account = get_theme_mod( 'cosme_enable_account', 1 );
		$cosme_enable_cart = get_theme_mod( 'cosme_enable_cart', 1 );
		?>
		<ul class="widget--header-items">
			<?php if( $cosme_enable_search ):?>
				<li class="search--widget-block">
					<!-- <?php get_search_form(); ?> -->
					<a href="search" aria-label="search form buttom" class="woocommerce_search_form_link">
 ...
 * Quantity buttons
 */
function cosme_woocommerce_before_quantity_input_field() {
	echo '<a href="#" class="cosme-quantity-minus" role="button">'. cosme_get_quantity_symbols_output( 'minus' ) .'</a>';
}
add_action( 'woocommerce_before_quantity_input_field', 'cosme_woocommerce_before_quantity_input_field' );

function cosme_woocommerce_after_quantity_input_field() {
	echo '<a href="#" class="cosme-quantity-plus" role="button">'.cosme_get_quantity_symbols_output( 'plus' ).'</a>';
}
add_action( 'woocommerce_after_quantity_input_field', 'cosme_woocommerce_after_quantity_input_field' );

function cosme_get_quantity_symbols_output( $type = 'plus' ) {
	if( $type === 'plus' ) {
		return '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-plus">
		<line x1="12" y1="5" x2="12" y2="19"></line>
		<line x1="5" y1="12" x2="19" y2="12"></line>
	</svg>';
	} else {
		return '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-minus">
		<line x1="5" y1="12" x2="19" y2="12"></line>
	</svg>';
	}	
}
